...
void dijet_xs() {
	...
	TFile* newfile = new TFile("correctedJES.root","RECREATE");
	...
	// arrays to store JER, JES, errJES, JESup, JESdown values
	double JER[16] = {0.0};
	double JES[16] = {0.0};
	double errJES[16] = {0.0};
	double JESup[16] = {0.0};
	double JESdown[16] = {0.0};
	double coljes1 = 0.0;
	double coljes2 = 0.0;
	double coljes3 = 0.0;
	double coljer1 = 0.0;
	...
	while (infile1 >> coljes1 >> coljes2 >> coljes3) {
	...
	for (int n=0; n<16; n++) {
		...
		JESup[n] = JES[n] + errJES[n];
		JESdown[n] = JES[n] - errJES[n];
		
		cout << "\n    JER of bin " << n+1 << " = " << JER[n] << endl;
		cout << "    JES of bin " << n+1 << " = " << JES[n] << " ± " << errJES[n] << "\n  JESup of bin " << n+1 << " = " << JESup[n] << "\nJESdown of bin " << n+1 << " = " << JESdown[n] << endl;
		...
		for (int i=0; i<t_mc->GetEntries(); i++) {
			...
			jet1ptUP_mc_corr = RanGen->Gaus((jet1pt_mc*JESup[n]),JER[n]*(jet1pt_mc*JESup[n]));
			newtree->GetBranch("jet1ptUP_mc_corr")->Fill();
			
			jet2ptUP_mc_corr = RanGen->Gaus((jet2pt_mc*JESup[n]),JER[n]*(jet2pt_mc*JESup[n]));
			newtree->GetBranch("jet2ptUP_mc_corr")->Fill();
			
			jet1ptDOWN_mc_corr = RanGen->Gaus((jet1pt_mc*JESdown[n]),JER[n]*(jet1pt_mc*JESdown[n]));
			newtree->GetBranch("jet1ptDOWN_mc_corr")->Fill();
			
			jet2ptDOWN_mc_corr = RanGen->Gaus((jet2pt_mc*JESdown[n]),JER[n]*(jet2pt_mc*JESdown[n]));
	...
	double eff_JESup = NselUP/Ntot;
	double eff_JESdown = NselDOWN/Ntot;
	
	double Delta_eff_JESup = abs(eff_ALL-eff_JESup);
	double Delta_eff_JESdown = abs(eff_ALL-eff_JESdown);
	
	cout << "\n     Ntot = " << Ntot << "\n     Nsel = " << Nsel << "\n   NselUP = " << NselUP << "\n NselDOWN = " << NselDOWN << endl;
	cout << "\n     εALL = " << eff_ALL << "\n   εJESup = " << eff_JESup << "\n εJESdown = " << eff_JESdown << endl;
	cout << "\n  ΔεJESup = " << Delta_eff_JESup << "\nΔεJESdown = " << Delta_eff_JESdown << endl;
